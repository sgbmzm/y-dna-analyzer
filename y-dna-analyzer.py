...
def ask_user_sample(sample_names):
    """פותח חלון עם Combobox לבחירת דגימה ומחזיר את הבחירה. מיועד ל VCF שיש בו כמה נדגמים"""
...
def load_user_dna_file():
    ...
    try:
        ...
        ###############################################################################
        # בדיקה האם זה vcf הכולל דגימות של נדגמים רבים.
        # אם כן נותנים למשתמש לבחור נדגם אחד מהם שעליו להמשיך להריץ את התוכנה
        if is_vcf_file:
            
            # פותח את הקובץ ובודק האם יש בו כמה דגימות ובכל מקרה אוסף את כל שמות הדגימות שיש בקובץ
            ...
            if len(sample_names) >1:
                ...
                # קובע את הטור של הנדגם בתוך ה vcf ואם לא אז 0 זה הטור הראשון כלומר הנדגם הראשון
    ...
    # פתיחת הקובץ וקריאת כל השורות לקבוע איזה ווריאנטים יש בכרומוזום Y
        with universal_opener(file_path, only_dna_if_zip=True) as f:
            header_found = False
            for line in f:
                line = line.strip()
                ...
                if is_vcf_file:
                    ...
                    ####################################################################
                    # --- טיפול מיוחד במקרה של ALT="." בקבצי big-y של ftdna---
                    if ad_str is not None:
                        # אם יש רק מספר אחד, כלומר רק REF
                        if "," not in ad_str and alt == ".":
                            ad_str = f"{ad_str},0"  # תוסיף ",0" בשביל לשמור פורמט אחיד
                    ####################################################################
                ...
                # נרמול שם הכרומוזום כדי שיהיה אפשר לבדוק באופן אחיד שמדובר בשורה של כרומוזום Y
...
def run_calculate_clade(Final_clade_index = 0):
    ...
    try:
        ...
        if not clades:
            ...
            result_var.set("All variants entered are not currently on the Yfull tree.\nTherefore, it is not possible to determine a location on the tree.")
            return
...
def save_clades_to_file():
    ...
    file_path = filedialog.asksaveasfilename(
        initialfile=f"{os.path.basename(last_dna_file) if last_dna_file else getattr(last_clades[0], 'name', '')}_yclade_results.txt",
